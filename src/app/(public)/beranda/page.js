...
// KOMPONEN: Slider Gambar untuk Kartu
...
export default function Beranda() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Ambil 4 berita terbaru agar pas di layout grid-cols-2
  ...
  const openModal = async (item, type) => { 
    ...
    if (type === "berita" && item.kategori === "LOMBA TERBUKA") {
      ...
      // FETCH KOMENTAR UNTUK BERITA TERSEBUT
      setKomentarList([]);
      try {
  ...
  // FUNGSI SUBMIT PENDAFTARAN LOMBA
  ...
  return (
    <div className="bg-[#f9f8f6] font-lora overflow-x-hidden relative">
      ...
      {/* MODAL POP-UP (SINKRON DENGAN HALAMAN MEDIA & PUBLIKASI) */}
      {selectedItem && (
      ...
      {/* KABAR TERBARU (SAMA PERSIS DENGAN HALAMAN MEDIA PUBLIKASI) */}
