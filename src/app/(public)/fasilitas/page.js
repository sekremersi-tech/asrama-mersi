...
// AutoSlider untuk Fasilitas & Sewa
const AutoSliderCard = ({ images, className, children, onClick }) => {
  ...
  if (imgArray.length === 0) return <div className={`bg-stone-200 ${className}`} onClick={onClick}>{children}</div>;
  return (
...
export default function FasilitasAsrama() {
  ...
  // STATE KOMENTAR & POP-UP UNTUK PENYEWAAN
  ...
  return (
    <div className="bg-[#f9f8f6] pb-24 min-h-screen text-left font-lora overflow-x-hidden relative">
      ...
      {/* MODAL POP-UP DETAIL PENYEWAAN & KOMENTAR */}
      {selectedItem && (
      ...
      {/* HERO & STATUS (SAMA SEPERTI SEBELUMNYA) */}
        ...
        {loading ? <p className="text-center py-20 text-stone-500 w-full">Memuat data...</p> : dataPenyewaan.length === 0 ? <div className="bg-white p-12 rounded-sm border border-amber-200 text-stone-500 text-center shadow-sm w-full">Belum ada layanan yang ditambahkan.</div> : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
            {dataPenyewaan.map(item => (
              <div key={item.id} onClick={() => openModal(item)} className="bg-white rounded-sm shadow-[4px_4px_0px_0px_rgba(245,158,11,0.1)] border border-amber-200 overflow-hidden group flex flex-col transition-all duration-300 w-full text-left relative cursor-pointer hover:-translate-y-1">
