...
export default function FasilitasAsrama() {
  ...
  // STATE MODAL INFORMASI FASILITAS & PENYEWAAN
  ...
  // FUNGSI MEMBUKA MODAL FASILITAS ATAU PENYEWAAN
  const openModal = async (item, type) => { 
    ...
    // Jika yang diklik adalah Penyewaan, fetch komentarnya
    if (type === "sewa") {
      try {
  ...
  return (
    <div className="bg-[#f9f8f6] pb-24 min-h-screen text-left font-lora overflow-x-hidden relative">
      ...
      {/* MODAL POP-UP DETAIL FASILITAS / PENYEWAAN */}
      {selectedItem && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-[fadeIn_0.3s_ease-out]" onClick={closeModal}>
          ...
          <div className="bg-white w-full max-w-5xl max-h-[90vh] rounded-sm overflow-hidden flex flex-col md:flex-row shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="w-full md:w-2/5 p-8 md:p-10 flex flex-col bg-[#fcfbf9] overflow-y-auto max-h-[50vh] md:max-h-[80vh]">
              {modalType === "sewa" ? (
                // TAMPILAN JIKA YANG DIKLIK ADALAH PENYEWAAN
                <>
                ...
                // TAMPILAN JIKA YANG DIKLIK ADALAH FASILITAS UMUM
                <>
      ...
      {/* 1. BAGIAN FASILITAS ASRAMA (DI ATAS) */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
            {dataFasilitas.map(item => (
              <div key={item.id} className="bg-white rounded-sm shadow-[4px_4px_0px_0px_rgba(23,20,18,0.05)] border border-[#e8e4db] overflow-hidden flex flex-col transition-all duration-300 w-full text-left">
                {/* KLIK FOTO UNTUK BUKA MODAL FASILITAS */}
      ...
      {/* 2. BAGIAN LAYANAN & PENYEWAAN (DI BAWAH FASILITAS) */}
        ...
        {loading ? <p className="text-center py-20 text-stone-500 w-full">Memuat data...</p> : dataPenyewaan.length === 0 ? <div className="bg-white p-12 rounded-sm border border-amber-200 text-stone-500 text-center shadow-sm w-full">Belum ada layanan yang ditambahkan.</div> : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
            {dataPenyewaan.map(item => (
              <div key={item.id} className="bg-white rounded-sm shadow-[4px_4px_0px_0px_rgba(245,158,11,0.1)] border border-amber-200 overflow-hidden flex flex-col transition-all duration-300 w-full text-left relative">
                ...
                {/* KLIK FOTO UNTUK BUKA MODAL PENYEWAAN (TERMASUK KOMENTAR) */}
