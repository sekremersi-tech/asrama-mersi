...
export default function FasilitasAsrama() {
  ...
  // STATE MODAL PENDAFTARAN ASRAMA
  ...
  const handleSubmitDaftar = async (e) => {
    ...
    try {
      await addDoc(collection(db, "pendaftaran_asrama"), {
        ...formDaftar, waktuDaftar: serverTimestamp()
      });
  ...
  return (
    <div className="bg-[#f9f8f6] pb-24 min-h-screen text-left font-lora overflow-x-hidden relative">
      ...
      {/* MODAL POP-UP PENDAFTARAN ASRAMA */}
      {showDaftarModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-[fadeIn_0.3s_ease-out]" onClick={closeDaftarModal}>
          ...
          <div className="bg-white w-full max-w-5xl max-h-[90vh] rounded-sm overflow-hidden flex flex-col md:flex-row shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            {/* Sisi Kiri: Brosur Penerimaan Warga */}
            <div className="relative w-full md:w-1/2 bg-stone-900 h-64 md:h-[80vh] flex flex-col items-center justify-center shrink-0">
              {brosurUrl ? (
                ...
                <div className="text-white text-center p-8">
                  <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" className="mx-auto mb-4 text-stone-600"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>
                  <p className="text-stone-400">Brosur belum diunggah admin.</p>
                </div>
              )}
            </div>
              ...
              <div className="flex flex-col h-full">
                ...
                <form onSubmit={handleSubmitDaftar} className="space-y-4 font-sans">
                  <div>
                    ...
                    <input type="text" required value={formDaftar.nama} onChange={(e) => setFormDaftar({...formDaftar, nama: e.target.value.replace(/[^a-zA-Z\s]/g, '')})} className="w-full px-4 py-2 bg-white border border-stone-200 rounded focus:ring-2 focus:ring-amber-500 focus:outline-none text-sm text-stone-900" placeholder="Hanya huruf..." />
                  </div>
                  
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-bold text-stone-800 uppercase tracking-widest block mb-1">Daerah Asal</label>
                      <input type="text" required value={formDaftar.asal} onChange={(e) => setFormDaftar({...formDaftar, asal: e.target.value})} className="w-full px-4 py-2 bg-white border border-stone-200 rounded focus:ring-2 focus:ring-amber-500 focus:outline-none text-sm text-stone-900" placeholder="Cth: Padang..." />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-stone-800 uppercase tracking-widest block mb-1">Suku</label>
                      <input type="text" required value={formDaftar.suku} onChange={(e) => setFormDaftar({...formDaftar, suku: e.target.value})} className="w-full px-4 py-2 bg-white border border-stone-200 rounded focus:ring-2 focus:ring-amber-500 focus:outline-none text-sm text-stone-900" placeholder="Cth: Minang..." />
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-bold text-stone-800 uppercase tracking-widest block mb-1">Kampus / Univ</label>
                      <input type="text" required value={formDaftar.kuliah} onChange={(e) => setFormDaftar({...formDaftar, kuliah: e.target.value})} className="w-full px-4 py-2 bg-white border border-stone-200 rounded focus:ring-2 focus:ring-amber-500 focus:outline-none text-sm text-stone-900" placeholder="Singkatan..." />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-stone-800 uppercase tracking-widest block mb-1">Jurusan / Prodi</label>
                      <input type="text" required value={formDaftar.jurusan} onChange={(e) => setFormDaftar({...formDaftar, jurusan: e.target.value})} className="w-full px-4 py-2 bg-white border border-stone-200 rounded focus:ring-2 focus:ring-amber-500 focus:outline-none text-sm text-stone-900" placeholder="Jurusan..." />
                    </div>
                  </div>

                  <div>
                    <label className="text-xs font-bold text-stone-800 uppercase tracking-widest block mb-1">Email Aktif</label>
                    <input type="email" required value={formDaftar.email} onChange={(e) => setFormDaftar({...formDaftar, email: e.target.value})} className="w-full px-4 py-2 bg-white border border-stone-200 rounded focus:ring-2 focus:ring-amber-500 focus:outline-none text-sm text-stone-900" placeholder="email@contoh.com" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-stone-800 uppercase tracking-widest block mb-1">Nomor HP / WhatsApp</label>
                    <input type="tel" required value={formDaftar.noHp} onChange={(e) => setFormDaftar({...formDaftar, noHp: e.target.value.replace(/\D/g, '')})} className="w-full px-4 py-2 bg-white border border-stone-200 rounded focus:ring-2 focus:ring-amber-500 focus:outline-none text-sm text-stone-900" placeholder="Awali dengan 08..." maxLength={14} />
                  </div>
      ...
      <HeroSlider images={bgFasilitas} title="Fasilitas Asrama" subtitle="Ruang nyaman dan sarana memadai untuk menunjang kehidupan serta produktivitas warga perantau." />
      ...
      {/* INFORMASI PENDAFTARAN (TATA TERTIB DIHAPUS, DIGANTI TOMBOL MODAL) */}
      ...
      <div className="max-w-7xl mx-auto px-4 mt-20 w-full text-left reveal opacity-0 translate-y-12 transition-all duration-1000 ease-out">
        <div className="flex items-center gap-4 mb-10 border-t border-[#e8e4db] pt-16">
          ...
          <div><h2 className="text-3xl font-bold text-stone-900 font-playfair">Daftar Fasilitas</h2><p className="text-stone-500 text-sm mt-1">Area fungsional untuk menunjang aktivitas warga.</p></div>
        </div>
