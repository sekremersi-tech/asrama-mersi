...
export default function FasilitasAsrama() {
  ...
  const [dataPenyewaan, setDataPenyewaan] = useState([]); // DITAMBAHKAN
  ...
  const formatWhatsAppLink = (nomor, namaSewa) => {
    ...
    const pesan = `Halo Uda/Uni, saya pengunjung website Asrama Merapi Singgalang. Saya ingin bertanya tentang informasi penyewaan *${namaSewa}*. Apakah bisa dibantu?`;
  ...
  return (
    <div className="bg-[#f9f8f6] pb-24 min-h-screen text-left font-lora overflow-x-hidden relative">
            ...
            <div className="w-full md:w-1/2 p-6 md:p-10 flex flex-col bg-[#fcfbf9] overflow-y-auto max-h-[50vh] md:max-h-[80vh]">
              <div className="flex flex-col h-full">
                ...
                <div className="mb-6 pb-4 border-b border-[#e8e4db]">
                  ...
                  <ul className="text-xs text-stone-500 font-sans list-disc list-inside ml-4 space-y-1"><li>Mahasiswa aktif (asal Sumatera Barat diutamakan)</li><li>Berkelakuan baik & bersedia mengikuti sistem asrama</li><li>Mengisi data diri dengan valid dan benar</li></ul>
                </div>
      ...
      {/* 1. BAGIAN FASILITAS (TIDAK DISEWAKAN) */}
      <div className="max-w-7xl mx-auto px-4 mt-20 w-full text-left reveal opacity-0 translate-y-12 transition-all duration-1000 ease-out">
        <div className="flex items-center gap-4 mb-10 border-t border-[#e8e4db] pt-16">
          ...
          <div><h2 className="text-3xl font-bold text-stone-900 font-playfair">Fasilitas Asrama</h2><p className="text-stone-500 text-sm mt-1">Area penunjang keseharian warga asrama (Tidak Disewakan).</p></div>
        </div>
        
        {loading ? <p className="text-center py-20 text-stone-500 w-full">Memuat data...</p> : dataFasilitas.length === 0 ? <div className="bg-white p-12 rounded-sm border text-stone-500 text-center shadow-sm w-full max-w-3xl mx-auto">Belum ada fasilitas yang ditambahkan.</div> : (
      ...
      {/* 2. BAGIAN LAYANAN & PENYEWAAN (BARU) */}
      <div className="max-w-7xl mx-auto px-4 mt-20 mb-20 w-full text-left reveal opacity-0 translate-y-12 transition-all duration-1000 ease-out">
        <div className="flex items-center gap-4 mb-10 border-t border-[#e8e4db] pt-16">
          ...
          <div><h2 className="text-3xl font-bold text-stone-900 font-playfair">Layanan & Penyewaan</h2><p className="text-stone-500 text-sm mt-1">Talenta seni budaya dan sarana yang dapat disewa oleh publik.</p></div>
        </div>
        
        {loading ? <p className="text-center py-20 text-stone-500 w-full">Memuat data...</p> : dataPenyewaan.length === 0 ? <div className="bg-white p-12 rounded-sm border text-stone-500 text-center shadow-sm w-full max-w-3xl mx-auto">Belum ada layanan yang ditambahkan.</div> : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
            {dataPenyewaan.map(item => (
              <div key={item.id} className="bg-white rounded-sm shadow-[4px_4px_0px_0px_rgba(23,20,18,0.05)] border border-amber-200 overflow-hidden group flex flex-col transition-all duration-300 w-full text-left relative">
                
                {/* LABEL KATEGORI PENYEWAAN (Seni Budaya / Barang) */}
                ...
                <div className="p-8 flex flex-col flex-grow relative w-full text-left items-start justify-start">
                  <h3 className="font-bold text-stone-900 text-2xl mb-3 font-playfair m-0">{item.nama}</h3>
                  <p className="text-stone-600 text-base leading-relaxed flex-grow m-0 text-left mb-6">{item.deskripsi}</p>
                  
                  {/* TAMPILAN HARGA & TOMBOL HUBUNGI */}
                  <div className="w-full pt-6 border-t border-[#e8e4db] flex flex-col font-sans mt-auto">
