...
export default function FasilitasAsrama() {
  ...
  return (
    <div className="bg-stone-50 pb-24 min-h-screen text-left">
      
      {/* HEADER HERO */}
      <div className="relative py-24 md:py-32 w-full bg-[#171412] flex flex-col items-center justify-center overflow-hidden text-center">
        <div 
      ...
      <div className="max-w-7xl mx-auto px-4 mt-16 w-full text-left">
        {loading ? (
          <p className="text-center py-20 text-stone-500 w-full">Memuat data fasilitas...</p>
        ) : dataFasilitas.length === 0 ? (
          <div className="bg-white p-12 rounded-2xl border border-stone-200 text-stone-500 text-center shadow-sm w-full max-w-3xl mx-auto flex flex-col items-center">
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" className="mb-4 text-stone-300"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path><polyline points="9 22 9 12 15 12 15 22"></polyline></svg>
            Belum ada fasilitas yang ditambahkan. Silakan kelola melalui Dashboard Admin.
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
            {dataFasilitas.map(item => (
              <div key={item.id} className="bg-white rounded-2xl shadow-sm hover:shadow-xl border border-stone-200 overflow-hidden group flex flex-col transition-all duration-300 w-full text-left">
                ...
                <div className="p-8 flex flex-col flex-grow relative w-full text-left items-start justify-start">
                  ...
                  <h3 className="font-bold text-stone-900 text-2xl mb-3 font-serif m-0">{item.nama}</h3>
