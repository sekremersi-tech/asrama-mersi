    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      <div className="bg-slate-900 py-16">
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <p className="text-gray-400 text-lg">Rekam jejak, inovasi, dan potret keseharian warga Asrama Merapi Singgalang.</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
            {dataGaleri.map((item) => (
              <div key={item.id} className="relative h-64 rounded-xl overflow-hidden group shadow-md border border-gray-100">
                ...
                <h3 className="absolute bottom-6 left-6 font-bold text-xl drop-shadow-md" style={{ color: item.warna || '#ffffff' }}>
                  {item.judul}
                </h3>
              </div>
            ))}
