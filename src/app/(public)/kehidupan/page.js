...
// KOMPONEN: Slider Gambar untuk Kartu (AutoSliderCard)
const AutoSliderCard = ({ images, className, children, onClick }) => {
  ...
  useEffect(() => {
    if (imgArray.length <= 1) return;
    const timer = setInterval(() => setIdx((prev) => (prev + 1) % imgArray.length), 3500);
    return () => clearInterval(timer);
  }, [imgArray.length]);
...
export default function Kehidupan() {
  const [bgMedia, setBgMedia] = useState([]); // Diubah menjadi Array
  ...
  const openModal = (item, type) => { setSelectedItem(item); setModalType(type); setModalImageIdx(0); document.body.style.overflow = "hidden"; };
  const closeModal = () => { setSelectedItem(null); document.body.style.overflow = "auto"; };
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-[fadeIn_0.3s_ease-out]" onClick={closeModal}>
          ...
          <div className="bg-white w-full max-w-5xl max-h-[90vh] rounded-sm overflow-hidden flex flex-col md:flex-row shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="w-full md:w-2/5 p-8 md:p-10 flex flex-col bg-[#fcfbf9] overflow-y-auto max-h-[50vh] md:max-h-[80vh]">
              ...
              {modalType === "berita" ? <p className="text-stone-700 leading-relaxed text-base whitespace-pre-line">{selectedItem.deskripsi}</p> : <p className="text-stone-500 italic">Dokumentasi momen kebersamaan warga Asrama Mahasiswa Merapi Singgalang.</p>}
            </div>
          </div>
        </div>
      )}
