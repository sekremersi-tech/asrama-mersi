...
export default function ProfilAsrama() {
  ...
  const [isAnimasi, setIsAnimasi] = useState(false); // State untuk Trigger Animasi Fade-in Teks Sejarah
  ...
  const changePage = (newIndex) => {
    if (newIndex >= 0 && newIndex < halamanSejarah.length) {
      ...
      setTimeout(() => {
        ...
        setIsAnimasi(false);
      }, 300); // Durasi fade-out sebelum teks berubah
    }
  };
  ...
  return (
    <div className="bg-[#f9f8f6] pb-24 font-lora overflow-x-hidden">
      ...
      {/* 1. SEJARAH (BUKU BERTUMPUK DENGAN ANIMASI FADE-IN TEXT) */}
        ...
        <div className="relative mt-8">
          ...
          <div className="relative bg-[#fcfbf9] p-8 md:p-14 rounded-sm shadow-2xl border border-[#e8e4db] z-10 flex flex-col min-h-[400px]">
            ...
            <div className="flex-grow flex items-center overflow-hidden">
              <p className={`text-stone-700 leading-relaxed text-lg md:text-xl text-justify whitespace-pre-line font-lora transition-all duration-300 transform ${isAnimasi ? 'opacity-0 translate-x-4' : 'opacity-100 translate-x-0'}`}>
            ...
            <div className="mt-12 flex justify-between items-center text-sm font-bold tracking-widest font-sans uppercase">
              <button onClick={() => changePage(halAktif - 1)} disabled={halAktif === 0} className={`flex items-center gap-2 transition-colors ${halAktif === 0 ? 'text-stone-300 cursor-not-allowed' : 'text-stone-500 hover:text-red-800'}`}>← Balik Lembar</button>
              <span className="text-stone-400 font-serif italic text-base lowercase">{halAktif + 1} / {halamanSejarah.length || 1}</span>
              <button onClick={() => changePage(halAktif + 1)} disabled={halAktif === halamanSejarah.length - 1} className={`flex items-center gap-2 transition-colors ${halAktif === halamanSejarah.length - 1 ? 'text-stone-300 cursor-not-allowed' : 'text-stone-900 hover:text-amber-600'}`}>Lanjut Baca →</button>
            </div>
      ...
      {/* 5. TITIK TEMU (MAPS DIKECILKAN SEPERTI DESAIN AWAL) */}
      <div id="lokasi" className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-32 mb-10 scroll-mt-28 reveal opacity-0 translate-y-12 transition-all duration-1000 ease-out">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-16 lg:gap-8 bg-white p-10 md:p-16 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#e8e4db]">
          ...
          <div className="w-full lg:w-1/2 relative flex justify-center lg:justify-end">
            <div className="w-full max-w-md h-[400px] rounded-3xl overflow-hidden shadow-2xl relative z-10 border-4 border-white transform transition-transform duration-500 hover:scale-[1.02] bg-stone-200">
              <iframe 
