...
export default function AdminDashboard() {
  ...
  // STATE KEPENGURUSAN ASRAMA (YANG SEMPAT HILANG)
  ...
  // FUNGSI KHUSUS MANAJEMEN KEPENGURUSAN
  const handleSavePengurusInti = async (e) => {
    ...
    try {
      ...
      setPengurusInti(newData); setFileInti({ ketua: null, sekretaris: null, bendahara: null });
  ...
  const handleTambahDivisi = async (e) => {
    ...
    try {
      ...
      setNamaDivisiBaru(""); fetchAllData();
    } catch (error) { setStatus({ type: "error", message: error.message }); } finally { setLoading(false); }
  };
  ...
  const handleTambahAnggota = async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, "anggota_divisi"), { divisiId: formAnggota.divisiId, nama: formAnggota.nama, foto: fotoUrl, createdAt: serverTimestamp() });
      setStatus({ type: "success", message: "Anggota Divisi berhasil ditambahkan!" });
      setFormAnggota({ ...formAnggota, nama: "" }); setFileAnggota(null); fetchAllData();
    } catch (error) { setStatus({ type: "error", message: error.message }); } finally { setLoading(false); }
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex space-x-2 mb-8 bg-white p-2 rounded-xl shadow-sm border border-slate-200 overflow-x-auto">
          ...
          {/* TAB STRUKTUR KEPENGURUSAN ADA DI SINI */}
          <button onClick={() => { setActiveTab("kepengurusan"); setStatus({}); }} className={`px-4 py-2 text-sm rounded-lg font-medium whitespace-nowrap ${activeTab === "kepengurusan" ? "bg-red-50 text-red-800 border border-red-200" : "text-slate-700 hover:bg-slate-50"}`}>Struktur Kepengurusan</button>
