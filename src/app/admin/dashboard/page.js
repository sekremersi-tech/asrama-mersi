  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-20">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        {/* TAB GALERI */}
        {activeTab === "galeri" && allowedTabs.includes("galeri") && ( <div className="space-y-6"> <div className="bg-white rounded-xl shadow-md p-6"> <h2 className="text-lg font-bold mb-4 border-b pb-2">{editGaleriId ? "Edit Galeri" : "Tambah Galeri Kegiatan"}</h2> <form onSubmit={handleSubmitGaleri} className="space-y-4"> <input type="text" required value={judulGaleri} onChange={(e) => setJudulGaleri(e.target.value)} placeholder="Judul Kegiatan..." className="w-full px-4 py-2 border rounded-md" />  <input type="color" value={warnaGaleri} onChange={(e) => setWarnaGaleri(e.target.value)} className="h-10 cursor-pointer border rounded-md" /> <input type="file" multiple accept="image/*" required={!editGaleriId} onChange={(e) => setFilesGaleri(Array.from(e.target.files))} className="w-full text-sm border p-2 rounded bg-slate-50" /> <div className="flex gap-2"><button type="submit" disabled={loading} className="w-full bg-slate-900 text-white px-4 py-2 rounded-md">{editGaleriId ? "Simpan Perubahan" : "Tambahkan"}</button>{editGaleriId && <button type="button" onClick={()=>{setEditGaleriId(null); setJudulGaleri(""); setWarnaGaleri("#ffffff");}} className="w-full bg-stone-500 text-white px-4 py-2 rounded-md">Batal</button>}</div> </form> </div> <div className="bg-white rounded-xl shadow-md p-6"> <h3 className="font-bold mb-4 border-b pb-2">Daftar Foto Galeri</h3> <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> {dataGaleri.slice((pageGaleri-1)*itemsPerPage, pageGaleri*itemsPerPage).map(item => ( <div key={item.id} className="relative h-40 rounded-lg overflow-hidden border"><img src={Array.isArray(item.linkGambar) ? item.linkGambar[0] : item.linkGambar} className="w-full h-full object-cover" /> <div className="absolute inset-0 bg-black/50 flex flex-col justify-end p-3"><span className="font-bold" style={{ color: item.warna }}>{item.judul}</span><div className="flex gap-2"><button onClick={()=>handleEditGaleriClick(item)} className="bg-white text-stone-900 text-xs px-3 py-1 rounded font-bold">Edit</button><button onClick={()=>handleDelete("fasilitas", item.id)} className="bg-red-600 text-white text-xs px-3 py-1 rounded">Hapus</button></div></div></div> ))} </div> <Pagination totalItems={dataGaleri.length} itemsPerPage={itemsPerPage} currentPage={pageGaleri} setCurrentPage={setPageGaleri}/> </div> </div> )}
...
pada bagian tab fasilitas foto yang ditambah tidak dapat tertampil di bagian user, 
saya sudah menambah 4 buah foto, padahal untuk  bagian lain dengan format penambahan dan file foto yang mirip dapat tertampil secara sempurna di bagian user, di admin ada daftar tapi fotonya gaada juga
pada bagian suara alumni dan galeri foto udah bisa tertampil dengan baik di user, apa masalahnya?
apakah ada kesalahan pada program fasilitasnya?
