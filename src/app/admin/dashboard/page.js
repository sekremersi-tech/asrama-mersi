...
export default function AdminDashboard() {
  ...
  // STATE KHUSUS PENYEWAAN (TERMASUK NO HP RESERVASI)
  ...
  const [noHpSewa, setNoHpSewa] = useState(""); // BARU: Nomor HP Reservasi
  ...
  // FUNGSI SUBMIT PENYEWAAN (DENGAN NOMOR HP)
  const handleSubmitPenyewaan = async (e) => {
    e.preventDefault(); setLoading(true);
    try {
      let urls = [];
      for (const file of filesSewa) { urls.push(await uploadToCloudinary(file, "image")); }
      await addDoc(collection(db, "daftar_penyewaan"), { 
        nama: namaSewa, 
        kategori: kategoriSewa, 
        harga: hargaSewa, 
        noHpSewa: noHpSewa, // MENYIMPAN NO HP RESERVASI
        deskripsi: deskripsiSewa, 
        linkGambar: urls, 
        createdAt: serverTimestamp() 
      });
      setStatus({ type: "success", message: "Item penyewaan berhasil ditambahkan!" });
      setNamaSewa(""); setDeskripsiSewa(""); setKategoriSewa("Tempat / Barang"); setHargaSewa(""); setNoHpSewa(""); setFilesSewa([]); e.target.reset(); fetchAllData();
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex space-x-2 mb-8 bg-white p-2 rounded-xl shadow-sm border border-slate-200 overflow-x-auto">
          <button onClick={() => { setActiveTab("tampilan"); setStatus({}); }} className={`px-4 py-2 text-sm rounded-lg font-medium whitespace-nowrap ${activeTab === "tampilan" ? "bg-red-50 text-red-800" : "text-slate-700 hover:bg-slate-50"}`}>Pengaturan Teks</button>
          <button onClick={() => { setActiveTab("status"); setStatus({}); }} className={`px-4 py-2 text-sm rounded-lg font-medium whitespace-nowrap ${activeTab === "status" ? "bg-red-50 text-red-800" : "text-slate-700 hover:bg-slate-50"}`}>Pendaftaran & Status</button>
          <button onClick={() => { setActiveTab("timeline"); setStatus({}); }} className={`px-4 py-2 text-sm rounded-lg font-medium whitespace-nowrap ${activeTab === "timeline" ? "bg-red-50 text-red-800" : "text-slate-700 hover:bg-slate-50"}`}>Timeline</button>
          ...
          <button onClick={() => { setActiveTab("penyewaan"); setStatus({}); }} className={`px-4 py-2 text-sm rounded-lg font-medium whitespace-nowrap ${activeTab === "penyewaan" ? "bg-amber-50 text-amber-800 border border-amber-200" : "text-slate-700 hover:bg-slate-50"}`}>Layanan Penyewaan</button>
          <button onClick={() => { setActiveTab("galeri"); setStatus({}); }} className={`px-4 py-2 text-sm rounded-lg font-medium whitespace-nowrap ${activeTab === "galeri" ? "bg-red-50 text-red-800" : "text-slate-700 hover:bg-slate-50"}`}>Galeri</button>
          <button onClick={() => { setActiveTab("kehidupan"); setStatus({}); }} className={`px-4 py-2 text-sm rounded-lg font-medium whitespace-nowrap ${activeTab === "kehidupan" ? "bg-red-50 text-red-800" : "text-slate-700 hover:bg-slate-50"}`}>Publikasi</button>
          <button onClick={() => { setActiveTab("skripsi"); setStatus({}); }} className={`px-4 py-2 text-sm rounded-lg font-medium whitespace-nowrap ${activeTab === "skripsi" ? "bg-red-50 text-red-800" : "text-slate-700 hover:bg-slate-50"}`}>Skripsi</button>
        ...
        {activeTab === "fasilitas" && (
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6">
              <h2 className="text-lg font-bold text-slate-900 mb-4 border-b border-slate-200 pb-2">Tambah Fasilitas Asrama Umum</h2>
              <form onSubmit={handleSubmitFasilitas} className="space-y-4">
                <input type="text" required value={namaFasilitas} onChange={(e) => setNamaFasilitas(e.target.value)} placeholder="Cth: Dapur Bersama..." className="w-full px-4 py-2 border rounded-md" />
                ...
                <input type="file" multiple required accept="image/*" onChange={(e) => setFilesFasilitas(Array.from(e.target.files))} className="w-full text-sm border p-2 rounded bg-slate-50 cursor-pointer" />
                <button type="submit" disabled={loading} className="w-full bg-slate-900 text-white px-4 py-2 rounded-md">Tambahkan Fasilitas</button>
              </form>
            </div>
            <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {dataFasilitas.map(item => {
                  ...
                  return (
                    <div key={item.id} className="bg-slate-50 border border-slate-200 rounded-lg flex flex-col overflow-hidden">
                      <img src={imgUtama} className="w-full h-32 object-cover" />
                      <div className="p-4 flex flex-col flex-grow">
                        <h4 className="font-bold mb-1 text-slate-900">{item.nama}</h4>
                        <p className="text-xs text-slate-600 flex-grow">{item.deskripsi}</p>
                        <button onClick={() => handleDelete("daftar_fasilitas", item.id)} className="bg-red-600 text-white text-xs px-3 py-1.5 mt-3 rounded">Hapus Fasilitas</button>
                      </div>
                    </div>
                  );
                })}
        ...
        {/* TAB LAYANAN PENYEWAAN DENGAN NOMOR HP SPESIFIK */}
        {activeTab === "penyewaan" && (
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-md border border-amber-200 p-6">
              <h2 className="text-lg font-bold text-slate-900 mb-4 border-b border-slate-200 pb-2 flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-amber-500"></span> Manajemen Layanan & Penyewaan
              </h2>
              <form onSubmit={handleSubmitPenyewaan} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-semibold mb-1 block">Nama Layanan / Keahlian / Barang</label>
                    <input type="text" required value={namaSewa} onChange={(e) => setNamaSewa(e.target.value)} placeholder="Cth: Grup Randai / Tari Piring / Aula..." className="w-full px-4 py-2 border rounded-md" />
                  </div>
                  <div>
                    <label className="text-sm font-semibold mb-1 block">Kategori Penyewaan</label>
                    <select value={kategoriSewa} onChange={(e) => setKategoriSewa(e.target.value)} className="w-full px-4 py-2 border rounded-md bg-white text-stone-900 font-medium">
                      <option value="Tempat / Barang">Tempat / Barang Fisik</option>
                      <option value="Keahlian Seni Budaya">Layanan Jasa & Keahlian Seni Budaya</option>
                    </select>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-semibold mb-1 block text-amber-600">Info Harga Sewa</label>
                    <input type="text" required value={hargaSewa} onChange={(e) => setHargaSewa(e.target.value)} placeholder="Cth: Rp 500.000 / Tampil" className="w-full px-4 py-2 border border-amber-300 bg-amber-50 rounded-md" />
                  </div>
                  <div>
                    <label className="text-sm font-semibold mb-1 block text-amber-600">No. WA Pengurus / Reservasi</label>
                    <input type="tel" required value={noHpSewa} onChange={(e) => setNoHpSewa(e.target.value.replace(/\D/g, ''))} placeholder="Cth: 08123456789 (Pengurus Randai)" className="w-full px-4 py-2 border border-amber-300 bg-amber-50 rounded-md" />
                  </div>
                </div>

                <textarea required rows="2" value={deskripsiSewa} onChange={(e) => setDeskripsiSewa(e.target.value)} placeholder="Deskripsi lengkap mengenai layanan ini..." className="w-full px-4 py-2 border rounded-md"></textarea>
                <input type="file" multiple required accept="image/*" onChange={(e) => setFilesSewa(Array.from(e.target.files))} className="w-full text-sm border p-2 rounded bg-slate-50 cursor-pointer" />
                <button type="submit" disabled={loading} className="w-full bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded-md font-bold transition-colors">Tambahkan Layanan Penyewaan</button>
              </form>
            </div>

            <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6">
              <h3 className="font-bold text-slate-900 mb-4 border-b border-slate-200 pb-2">Daftar Layanan Tersedia</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {dataPenyewaan.map(item => {
                  const imgUtama = Array.isArray(item.linkGambar) ? item.linkGambar[0] : item.linkGambar;
                  return (
                    <div key={item.id} className="bg-slate-50 border border-slate-200 rounded-lg flex overflow-hidden">
                      <img src={imgUtama} className="w-32 h-full object-cover shrink-0" />
                      <div className="p-4 flex flex-col w-full justify-center">
                        ...
                        <h4 className="font-bold text-stone-900 leading-tight">{item.nama}</h4>
                        {/* MENAMPILKAN NOMOR HP DI ADMIN PANEL */}
                        <p className="text-amber-600 text-xs font-bold my-1">{item.noHpSewa || "Nomor Belum Diatur"}</p>
                        <p className="text-xs text-stone-500 line-clamp-2 mt-1 mb-2">{item.deskripsi}</p>
                        <button onClick={() => handleDelete("daftar_penyewaan", item.id)} className="text-red-500 text-xs font-semibold self-start hover:underline">Hapus Layanan</button>
                      </div>
                    </div>
                  );
                })}
