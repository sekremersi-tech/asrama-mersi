...
export default function AdminDashboard() {
  ...
  const [kategoriFasilitas, setKategoriFasilitas] = useState("Fasilitas Umum"); // BARU: Pilihan kategori
  const [hargaSewa, setHargaSewa] = useState(""); // BARU: Info Harga
  ...
  const handleSubmitFasilitas = async (e) => {
    ...
    try {
      ...
      setStatus({ type: "success", message: "Item berhasil ditambahkan!" });
      setNamaFasilitas(""); setDeskripsiFasilitas(""); setKategoriFasilitas("Fasilitas Umum"); setHargaSewa(""); setFilesFasilitas([]); e.target.reset(); fetchAllData();
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex space-x-2 mb-8 bg-white p-2 rounded-xl shadow-sm border border-slate-200 overflow-x-auto">
          ...
          <button onClick={() => { setActiveTab("status"); setStatus({}); }} className={`px-4 py-2 text-sm rounded-lg font-medium whitespace-nowrap ${activeTab === "status" ? "bg-red-50 text-red-800" : "text-slate-700 hover:bg-slate-50"}`}>Status Asrama</button>
          ...
          <button onClick={() => { setActiveTab("fasilitas"); setStatus({}); }} className={`px-4 py-2 text-sm rounded-lg font-medium whitespace-nowrap ${activeTab === "fasilitas" ? "bg-red-50 text-red-800" : "text-slate-700 hover:bg-slate-50"}`}>Fasilitas & Sewa</button>
        ...
        {/* --- TAB LAINNYA DIHILANGKAN SEMENTARA (SAMA PERSIS SEPERTI SEBELUMNYA) --- */}
        ...
        {/* TAB FASILITAS & PENYEWAAN (DIPERBARUI) */}
        {activeTab === "fasilitas" && (
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6">
              <h2 className="text-lg font-bold text-slate-900 mb-4 border-b border-slate-200 pb-2">Manajemen Fasilitas & Penyewaan</h2>
              <form onSubmit={handleSubmitFasilitas} className="space-y-4">
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-semibold mb-1 block">Nama Tempat/Barang</label>
                    <input type="text" required value={namaFasilitas} onChange={(e) => setNamaFasilitas(e.target.value)} placeholder="Cth: Aula Asrama..." className="w-full px-4 py-2 border rounded-md" />
                  </div>
                  <div>
                    <label className="text-sm font-semibold mb-1 block">Kategori</label>
                    <select value={kategoriFasilitas} onChange={(e) => setKategoriFasilitas(e.target.value)} className="w-full px-4 py-2 border rounded-md bg-white">
                      <option value="Fasilitas Umum">Fasilitas Umum (Tidak Disewakan)</option>
                      <option value="Disewakan">Disewakan (Publik)</option>
                    </select>
                  </div>
                </div>

                {/* Info Harga Hanya Muncul Jika Kategori "Disewakan" */}
                {kategoriFasilitas === "Disewakan" && (
                  <div>
                    <label className="text-sm font-semibold mb-1 block text-amber-600">Info Harga Sewa</label>
                    <input type="text" required value={hargaSewa} onChange={(e) => setHargaSewa(e.target.value)} placeholder="Cth: Rp 50.000 / Hari" className="w-full px-4 py-2 border border-amber-300 bg-amber-50 rounded-md" />
                  </div>
                )}

                <textarea required rows="2" value={deskripsiFasilitas} onChange={(e) => setDeskripsiFasilitas(e.target.value)} placeholder="Deskripsi lengkap..." className="w-full px-4 py-2 border rounded-md"></textarea>
                ...
                <button type="submit" disabled={loading} className="w-full bg-slate-900 text-white px-4 py-2 rounded-md font-bold">Tambahkan Item</button>
              </form>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {dataFasilitas.map(item => {
                  ...
                  return (
                    <div key={item.id} className="bg-slate-50 border border-slate-200 rounded-lg flex flex-col relative overflow-hidden">
                      {item.kategori === "Disewakan" && (
                        <div className="absolute top-2 left-2 bg-amber-500 text-white text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider z-10 shadow-md">Disewakan</div>
                      )}
                      <img src={imgUtama} className="w-full h-32 object-cover" />
                      <div className="p-4 flex flex-col flex-grow">
                        <h4 className="font-bold">{item.nama}</h4>
                        {item.kategori === "Disewakan" && <p className="text-amber-600 text-xs font-bold my-1">{item.harga}</p>}
                        <p className="text-xs text-stone-600 flex-grow mt-1">{item.deskripsi}</p>
                        <button onClick={() => handleDelete("daftar_fasilitas", item.id)} className="bg-red-600 text-white text-xs px-3 py-1.5 mt-3 rounded">Hapus Item</button>
                      </div>
                    </div>
                  );
                })}
