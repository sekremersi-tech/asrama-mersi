...
const TAB_ROLES = {
  ...
  tendor: ["fotoprofil", "galeri", "kehidupan", "penyewaan"], 
...
export default function AdminDashboard() {
  ...
  const fetchAllData = async () => {
    ...
    // MENGAMBIL DATA BROSUR DAN LINK FORMULIR
  ...
  // PENYIMPANAN BROSUR DAN LINK FORMULIR
          ...
          <div className="space-y-6"> 
            ...
            <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6"> 
              ...
              <form onSubmit={handleSaveBrosur} className="space-y-6"> 
                <div>
                  <label className="text-sm font-semibold mb-1 block text-stone-700">Link Google Drive Formulir (Kosong)</label>
                  <input type="url" value={linkFormulir} onChange={(e) => setLinkFormulir(e.target.value)} placeholder="https://drive.google.com/..." className="w-full px-4 py-2 border border-slate-300 bg-slate-50 rounded-md text-sm" />
                  <p className="text-[10px] text-stone-500 mt-1">Link ini akan digunakan pada tombol "Unduh Formulir" di pop-up pendaftaran.</p>
                </div>
                <div>
                  <label className="text-sm font-semibold mb-1 block text-stone-700">Upload Gambar Brosur Baru</label>
                  <div className="bg-slate-50 p-4 border border-slate-200 rounded-lg flex flex-col md:flex-row md:items-center justify-between gap-4"> 
                    <input type="file" accept="image/*" onChange={(e) => setFileBrosur(e.target.files[0])} className="w-full text-sm cursor-pointer" /> 
                    {brosurUrl && <a href={brosurUrl} target="_blank" className="text-xs text-amber-600 font-bold whitespace-nowrap bg-amber-50 px-3 py-1.5 border border-amber-200 rounded">Lihat Brosur Saat Ini</a>} 
                  </div> 
                </div>
                <button type="submit" disabled={loading} className="w-full bg-slate-900 text-white px-4 py-2.5 rounded-md font-bold">Simpan Pengaturan Pendaftaran</button> 
              </form> 
            </div> 
          </div> 
        )}
