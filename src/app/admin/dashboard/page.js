...
export default function AdminDashboard() {
  ...
  const handleSaveProfilText = async (e) => {
    ...
    try {
      ...
      setStatus({ type: "success", message: "Teks profil berhasil diperbarui!" });
    } catch (error) { setStatus({ type: "error", message: error.message }); } finally { setLoading(false); }
  };
  ...
  const handleSubmitGaleri = async (e) => {
    ...
    try {
      if (!fileGaleri) throw new Error("Wajib unggah foto kegiatan!");
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex space-x-2 mb-8 bg-white p-2 rounded-xl shadow-sm border border-slate-200 overflow-x-auto">
          <button onClick={() => { setActiveTab("tampilan"); setStatus({}); }} className={`px-4 py-2 text-sm rounded-lg font-medium whitespace-nowrap ${activeTab === "tampilan" ? "bg-red-50 text-red-800" : "text-slate-700 hover:bg-slate-50"}`}>Pengaturan Profil & Foto</button>
        ...
        {/* TAB 1: PENGATURAN PROFIL & FOTO */}
        {activeTab === "tampilan" && (
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6">
              <h2 className="text-lg font-bold text-slate-900 mb-4 border-b border-slate-200 pb-2">Edit Teks Profil Asrama</h2>
              <form onSubmit={handleSaveProfilText} className="space-y-4">
                <div>
                ...
                <div>
                  ...
                  <textarea required rows="2" value={profilText.visi} onChange={(e) => setProfilText({...profilText, visi: e.target.value})} className="w-full px-4 py-2 border border-slate-300 bg-white text-slate-900 rounded-md focus:outline-none focus:ring-2 focus:ring-red-800 placeholder-slate-400"></textarea>
                </div>
                <div>
                  <label className="font-semibold text-slate-800 text-sm block mb-1">Misi</label>
                  <textarea required rows="3" value={profilText.misi} onChange={(e) => setProfilText({...profilText, misi: e.target.value})} className="w-full px-4 py-2 border border-slate-300 bg-white text-slate-900 rounded-md focus:outline-none focus:ring-2 focus:ring-red-800 placeholder-slate-400"></textarea>
                </div>
                <button type="submit" disabled={loading} className="bg-slate-900 text-white px-6 py-2 rounded-md font-semibold hover:bg-slate-800">{loading ? "Menyimpan..." : "Simpan Teks Profil"}</button>
              </form>
            </div>

            <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6">
              <h2 className="text-lg font-bold text-slate-900 mb-4 border-b border-slate-200 pb-2">Ubah Foto Utama</h2>
              <form onSubmit={handleSaveTampilan} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center border border-slate-200 bg-slate-50 p-4 rounded-lg">
                  <div>
                    <label className="font-semibold text-slate-800">Foto Latar Beranda (Hero)</label>
                    <p className="text-xs text-slate-500 mt-1">Aktif: {tampilanUrls.hero ? "Ya" : "Bawaan"}</p>
                  </div>
                  <input type="file" accept="image/*" onChange={(e) => setTampilanFiles({...tampilanFiles, hero: e.target.files[0]})} className="text-sm text-slate-700 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:bg-white file:border-slate-300 file:border cursor-pointer bg-transparent" />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center border border-slate-200 bg-slate-50 p-4 rounded-lg">
                  <div>
                    <label className="font-semibold text-slate-800">Foto Halaman Profil</label>
                    <p className="text-xs text-slate-500 mt-1">Aktif: {tampilanUrls.profil ? "Ya" : "Bawaan"}</p>
                  </div>
                  <input type="file" accept="image/*" onChange={(e) => setTampilanFiles({...tampilanFiles, profil: e.target.files[0]})} className="text-sm text-slate-700 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:bg-white file:border-slate-300 file:border cursor-pointer bg-transparent" />
                </div>
                <button type="submit" disabled={loading} className="bg-slate-900 text-white px-6 py-2 rounded-md font-semibold hover:bg-slate-800">{loading ? "Menyimpan..." : "Simpan Foto"}</button>
              </form>
            </div>
          </div>
        )}
            ...
            <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6">
              ...
              <form onSubmit={handleSubmitGaleri} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-[1fr_100px] gap-4">
                  <input type="text" required value={judulGaleri} onChange={(e) => setJudulGaleri(e.target.value)} placeholder="Nama/Judul Kegiatan..." className="px-4 py-2 border border-slate-300 text-slate-900 bg-white rounded-md focus:outline-none focus:ring-2 focus:ring-red-800 placeholder-slate-400" />
             ...
             <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6">
               ...
               <form onSubmit={handleSubmitKehidupan} className="space-y-4">
                 <div className="grid grid-cols-1 md:grid-cols-[1fr_200px] gap-4">
                   <input type="text" required value={judulKonten} onChange={(e) => setJudulKonten(e.target.value)} placeholder="Judul Berita..." className="px-4 py-2 border border-slate-300 text-slate-900 bg-white rounded-md focus:outline-none focus:ring-2 focus:ring-red-800 placeholder-slate-400" />
                 ...
                 <textarea required rows="3" value={deskripsi} onChange={(e) => setDeskripsi(e.target.value)} placeholder="Deskripsi..." className="w-full px-4 py-2 border border-slate-300 text-slate-900 bg-white rounded-md focus:outline-none focus:ring-2 focus:ring-red-800 placeholder-slate-400"></textarea>
           ...
           <div className="space-y-6">
             <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6">
               ...
               <form onSubmit={handleSubmitSkripsi} className="space-y-4">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   <input type="text" required value={nama} onChange={(e) => setNama(e.target.value)} placeholder="Nama Alumni..." className="px-4 py-2 border border-slate-300 text-slate-900 bg-white rounded-md focus:outline-none focus:ring-2 focus:ring-red-800 placeholder-slate-400" />
                   <input type="text" required value={jurusan} onChange={(e) => setJurusan(e.target.value)} placeholder="Jurusan & Univ..." className="px-4 py-2 border border-slate-300 text-slate-900 bg-white rounded-md focus:outline-none focus:ring-2 focus:ring-red-800 placeholder-slate-400" />
                 </div>
                 <div className="grid grid-cols-1 md:grid-cols-[1fr_120px] gap-4">
                   <textarea required rows="1" value={judulSkripsi} onChange={(e) => setJudulSkripsi(e.target.value)} placeholder="Judul Skripsi..." className="w-full px-4 py-2 border border-slate-300 text-slate-900 bg-white rounded-md focus:outline-none focus:ring-2 focus:ring-red-800 placeholder-slate-400"></textarea>
                   <input type="number" required value={tahun} onChange={(e) => setTahun(e.target.value)} placeholder="Tahun" className="px-4 py-2 border border-slate-300 text-slate-900 bg-white rounded-md focus:outline-none focus:ring-2 focus:ring-red-800 placeholder-slate-400" />
                 </div>
                 <div>
