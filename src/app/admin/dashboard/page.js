...
export default function AdminDashboard() {
  ...
  const handleTolakSkripsi = async (item) => {
    try { await updateDoc(doc(db, "permohonan_skripsi", item.id), { status: "Ditolak" }); fetchAllData(); } 
    catch (error) { alert("Gagal menolak."); }
  };

  const handleResetStatusSkripsi = async (id) => {
    try { await updateDoc(doc(db, "permohonan_skripsi", id), { status: "Menunggu" }); fetchAllData(); } 
          ...
          <div className="space-y-6"> 
            <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6"> 
              ...
              <form onSubmit={handleSaveStatusAsrama} className="space-y-4"> 
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4"> 
                  ...
                  <div> <label className="text-sm font-semibold mb-1 block">Ketersediaan</label> <select value={statusAsrama.ketersediaan} onChange={(e) => setStatusAsrama({...statusAsrama, ketersediaan: e.target.value})} className="w-full px-4 py-2 border rounded-md"> <option value="Tersedia">🟢 Tersedia</option> <option value="Penuh">🔴 Penuh</option> </select> </div> 
                </div> 
            ...
            <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6"> 
              ...
              <form onSubmit={handleSaveBrosur} className="space-y-6"> 
                <div> <label className="text-sm font-semibold mb-1 block">Link Google Drive Formulir (Kosong)</label> <input type="url" value={linkFormulir} onChange={(e) => setLinkFormulir(e.target.value)} className="w-full px-4 py-2 border rounded-md" /> </div> 
                <div> 
            ...
            <div className="bg-white rounded-xl shadow-md border-amber-200 p-6"> 
              ...
              <form onSubmit={handleSubmitPenyewaan} className="space-y-4"> 
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> 
                  ...
                  <div><label className="text-sm font-semibold mb-1 block">Kategori</label><select value={kategoriSewa} onChange={(e) => setKategoriSewa(e.target.value)} className="w-full px-4 py-2 border rounded-md"><option value="Tempat / Barang">Tempat / Barang Fisik</option><option value="Keahlian Seni Budaya">Layanan Jasa & Seni</option></select></div> 
                </div> 
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> 
            ...
            <div className="bg-white rounded-xl shadow-md p-6"> 
              ...
              <form onSubmit={handleSubmitKehidupan} className="space-y-4"> 
                ...
                <select value={kategori} onChange={(e) => setKategori(e.target.value)} className="w-full px-4 py-2 border rounded-md font-bold"> 
                  <option value="PRESTASI">Prestasi</option> <option value="MERSI X BK">MERSI X BK</option> <option value="LOMBA TERBUKA">Lomba Terbuka</option> <option value="LAINNYA">Lainnya... (Isi Manual)</option> 
                </select> 
