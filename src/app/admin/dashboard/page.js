...
export default function AdminDashboard() {
  ...
  // STATE KHUSUS PENYEWAAN (TERMASUK SENI BUDAYA)
  ...
  // FUNGSI SUBMIT FASILITAS UMUM (TANPA HARGA SEWA)
  ...
  // FUNGSI SUBMIT LAYANAN PENYEWAAN (DENGAN HARGA DAN KATEGORI SENI BUDAYA)
  const handleSubmitPenyewaan = async (e) => {
    e.preventDefault(); setLoading(true);
    try {
      let urls = [];
      for (const file of filesSewa) { urls.push(await uploadToCloudinary(file, "image")); }
      await addDoc(collection(db, "daftar_penyewaan"), { nama: namaSewa, kategori: kategoriSewa, harga: hargaSewa, deskripsi: deskripsiSewa, linkGambar: urls, createdAt: serverTimestamp() });
      setStatus({ type: "success", message: "Item penyewaan berhasil ditambahkan!" });
      setNamaSewa(""); setDeskripsiSewa(""); setKategoriSewa("Tempat / Barang"); setHargaSewa(""); setFilesSewa([]); e.target.reset(); fetchAllData();
    } catch (error) { setStatus({ type: "error", message: error.message }); } finally { setLoading(false); }
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex space-x-2 mb-8 bg-white p-2 rounded-xl shadow-sm border border-slate-200 overflow-x-auto">
          ...
          {/* TAB DIPISAH: Fasilitas Umum & Layanan Penyewaan */}
        ...
        {/* --- TAB FASILITAS ASRAMA (TANPA HARGA SEWA) --- */}
        {activeTab === "fasilitas" && (
        ...
        {/* --- TAB LAYANAN PENYEWAAN (KHUSUS SEWA & SENI BUDAYA) --- */}
        {activeTab === "penyewaan" && (
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-md border border-amber-200 p-6">
              ...
              <form onSubmit={handleSubmitPenyewaan} className="space-y-4">
                ...
                <div>
                  ...
                  <input type="text" required value={hargaSewa} onChange={(e) => setHargaSewa(e.target.value)} placeholder="Cth: Rp 500.000 / Tampil, atau 'Hubungi Kami'" className="w-full px-4 py-2 border border-amber-300 bg-amber-50 rounded-md" />
                </div>
                  ...
                  return (
                    <div key={item.id} className="bg-slate-50 border border-slate-200 rounded-lg flex overflow-hidden">
                      ...
                      <div className="p-4 flex flex-col w-full justify-center">
                        ...
                        <p className="text-amber-600 text-xs font-bold my-1">{item.harga}</p>
