...
export default function AdminDashboard() {
  ...
  // State: Tampilan Beranda & Profil (Foto)
  ...
  // State: Teks Profil (Sejarah, Visi, Misi)
  const [profilText, setProfilText] = useState({ sejarah: "", visi: "", misi: "" });

  // State: Galeri Kegiatan (sebelumnya Fasilitas)
  ...
  const fetchAllData = async () => {
    ...
    const galSnap = await getDocs(query(collection(db, "fasilitas"), orderBy("createdAt", "desc"))); // Tetap pakai koleksi 'fasilitas' agar data lama aman
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex space-x-2 mb-8 bg-white p-2 rounded-xl shadow-sm border border-slate-100 overflow-x-auto">
          <button onClick={() => { setActiveTab("tampilan"); setStatus({}); }} className={`px-4 py-2 text-sm rounded-lg font-medium whitespace-nowrap ${activeTab === "tampilan" ? "bg-red-50 text-red-800" : "text-slate-600 hover:bg-slate-50"}`}>Pengaturan Profil & Foto</button>
          <button onClick={() => { setActiveTab("galeri"); setStatus({}); }} className={`px-4 py-2 text-sm rounded-lg font-medium whitespace-nowrap ${activeTab === "galeri" ? "bg-red-50 text-red-800" : "text-slate-600 hover:bg-slate-50"}`}>Galeri Kegiatan</button>
          <button onClick={() => { setActiveTab("kehidupan"); setStatus({}); }} className={`px-4 py-2 text-sm rounded-lg font-medium whitespace-nowrap ${activeTab === "kehidupan" ? "bg-red-50 text-red-800" : "text-slate-600 hover:bg-slate-50"}`}>Kabar & Berita</button>
        ...
        {activeTab === "tampilan" && (
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6">
              <h2 className="text-lg font-bold mb-4 border-b pb-2">Edit Teks Profil Asrama</h2>
              <form onSubmit={handleSaveProfilText} className="space-y-4">
                <div><label className="font-medium text-sm block mb-1">Sejarah Asrama</label><textarea required rows="4" value={profilText.sejarah} onChange={(e) => setProfilText({...profilText, sejarah: e.target.value})} className="w-full px-4 py-2 border rounded-md"></textarea></div>
                <div><label className="font-medium text-sm block mb-1">Visi</label><textarea required rows="2" value={profilText.visi} onChange={(e) => setProfilText({...profilText, visi: e.target.value})} className="w-full px-4 py-2 border rounded-md"></textarea></div>
                <div><label className="font-medium text-sm block mb-1">Misi</label><textarea required rows="3" value={profilText.misi} onChange={(e) => setProfilText({...profilText, misi: e.target.value})} className="w-full px-4 py-2 border rounded-md"></textarea></div>
                <button type="submit" disabled={loading} className="bg-slate-900 text-white px-6 py-2 rounded-md font-semibold">{loading ? "Menyimpan..." : "Simpan Teks Profil"}</button>
              </form>
            </div>

            <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6">
              <h2 className="text-lg font-bold mb-4 border-b pb-2">Ubah Foto Utama</h2>
        ...
        {activeTab === "galeri" && (
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6">
              <h2 className="text-lg font-bold mb-4 border-b pb-2">Tambah Foto Galeri Kegiatan</h2>
              <form onSubmit={handleSubmitGaleri} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-[1fr_100px] gap-4">
                  <input type="text" required value={judulGaleri} onChange={(e) => setJudulGaleri(e.target.value)} placeholder="Nama/Judul Kegiatan..." className="px-4 py-2 border rounded-md" />
                  <div className="flex flex-col"><label className="text-xs text-slate-500 mb-1">Warna Teks</label><input type="color" value={warnaGaleri} onChange={(e) => setWarnaGaleri(e.target.value)} className="w-full h-8 cursor-pointer border rounded-md p-0.5" /></div>
                </div>
                ...
                <button type="submit" disabled={loading} className="w-full bg-slate-900 text-white px-4 py-2 rounded-md font-semibold">{loading ? "Menyimpan..." : "Tambah ke Galeri"}</button>
              </form>
            </div>
            <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {dataGaleri.map(item => (
                  <div key={item.id} className="relative h-40 rounded-lg overflow-hidden border">
        ...
        {/* TAB 3 & 4 (Berita & Skripsi) SAMA SEPERTI SEBELUMNYA */}
        {activeTab === "kehidupan" && (
           <div className="space-y-6">
             <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6">
           ...
           <div className="space-y-6">
             <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6">
               ...
               <form onSubmit={handleSubmitSkripsi} className="space-y-4">
                 ...
                 <div>
                   <label className="text-sm text-slate-500 block mb-1">Unggah File PDF</label>
