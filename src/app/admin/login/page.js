...
export default function AdminLogin() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#f9f8f6] p-4 font-lora relative">
      ...
      {/* PERBAIKAN TOMBOL KEMBALI DINAMIS (MENGGUNAKAN router.back()) */}
      <button 
        onClick={() => router.back()} 
        className="absolute top-6 left-6 flex items-center gap-2 text-stone-500 hover:text-red-800 font-bold transition-colors font-sans text-sm tracking-widest uppercase cursor-pointer z-10 bg-white/50 p-2 rounded-md backdrop-blur-sm shadow-sm"
      >
      ...
      <div className="bg-white p-8 md:p-12 rounded-xl shadow-2xl max-w-md w-full border border-[#e8e4db] text-center mt-12 md:mt-0 relative z-0">
        
        {/* DUA LOGO BERDAMPINGAN */}
        <div className="flex justify-center items-center mb-6">
          <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-md border border-stone-200 z-20">
            <img src="/mersi.png" alt="Logo Mersi" className="w-10 h-10 object-contain drop-shadow-sm" />
          </div>
          <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-md border border-stone-200 -ml-4 z-10">
            <img src="/BK.png" alt="Logo Bundo Kanduang" className="w-10 h-10 object-contain drop-shadow-sm" />
          </div>
        </div>

        <h1 className="text-3xl font-bold font-playfair text-stone-900 mb-2">Portal Admin</h1>
        <p className="text-stone-500 text-sm mb-8 px-4">Sistem Manajemen Informasi Asrama Mahasiswa Merapi Singgalang & Bundo Kanduang.</p>
